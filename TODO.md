...
- [x] **Design Health pre-export check** — `tools/design_health.py`, a single "would this network work if built?" go/no-go verdict surfaced as a "Health" button + banner in the validation dock. Aggregates the route/optical + FK-integrity headless validators plus a chamber-sizing scan, re-weighed under a stricter "would-it-work-if-built" lens (validate_routes left untouched): optical-fail / partial / orphaned-FK / oversubscription block (NO-GO); unserved premises caution as incomplete-not-broken (CAUTION); unsized chambers are cost-only (info, GO). Verified live on CH33 -> CAUTION (31/1189 routed, 0 errors, correctly distinguishing incomplete from broken). Regression test `tests/test_design_health.py` 8/8 (validators stubbed, no live data needed). Commit 65a0b4d.
